...
verify: bas10.rom bas11.rom bas12.rom bas13.rom extbas10.rom extbas11.rom disk10.rom disk11.rom coco3.rom coco3p.rom
	@echo "verifying all roms...  (no further output means all are ok)..."
	@# ============== "official" mess hashes
	@sha1sum bas10.rom       | grep "1f08455cd48ce6a06132aea15c4778f264e19539" >/dev/null || echo "bas10.rom      ***** BAD SHA1 *****"
	@sha1sum bas11.rom       | grep "cecb7c24ff1e0ab5836e4a7a8eb1b8e01f1fded3" >/dev/null || echo "bas11.rom      ***** BAD SHA1 *****"
	@sha1sum bas12.rom       | grep "0f14dc46c647510eb0b7bd3f53e33da07907d04f" >/dev/null || echo "bas12.rom      ***** BAD SHA1 *****"
	@sha1sum bas13.rom       | grep "28b92bebe35fa4f026a084416d6ea3b1552b63d3" >/dev/null || echo "bas13.rom      ***** BAD SHA1 *****"
	@sha1sum extbas10.rom    | grep "8aa58f2eb3e8bcfd5470e3e35e2b359e9a72848e" >/dev/null || echo "extbas10.rom:  ***** BAD SHA1 *****"
	@sha1sum extbas11.rom    | grep "ad927fb4f30746d820cb8b860ebb585e7f095dea" >/dev/null || echo "extbas11.rom:  ***** BAD SHA1 *****"
	@sha1sum disk10.rom      | grep "04115be3f97952b9d9310b52f806d04f80b40d03" >/dev/null || echo "disk10.rom:    ***** BAD SHA1 *****"
	@sha1sum disk11.rom      | grep "10bdc5aa2d7d7f205f67b47b19003a4bd89defd1" >/dev/null || echo "disk11.rom:    ***** BAD SHA1 *****"
	@sha1sum coco3.rom       | grep "e0d82953fb6fd03768604933df1ce8bc51fc427d" >/dev/null || echo "coco3.rom      ***** BAD SHA1 *****"
	@sha1sum coco3p.rom      | grep "631e383068b1f52a8f419f4114b69501b21cf379" >/dev/null || echo "coco3p.rom     ***** BAD SHA1 *****"
	@# ============== self-generated hashes (these are just portions of coco3.rom and coco3p.rom above)
	@sha1sum bas20.rom       | grep "09015df53738be4ae9e50a84f26a19b8582c05ff" >/dev/null || echo "bas20.rom      ***** BAD SHA1 *****"
	@sha1sum extbas20.rom    | grep "d322cbd9f959bdc5f2cba7baac37071450946b5e" >/dev/null || echo "extbas20.rom:  ***** BAD SHA1 *****"
	@sha1sum supbas20.rom    | grep "9a05652fd80ab87219102800fe6a44a4ecacd8c3" >/dev/null || echo "supbas20.rom:  ***** BAD SHA1 *****"
	@sha1sum supbas20p.rom   | grep "1953f7c33f0588ac975212d8ec3c6faee4ebad39" >/dev/null || echo "supbas20p.rom: ***** BAD SHA1 *****"
